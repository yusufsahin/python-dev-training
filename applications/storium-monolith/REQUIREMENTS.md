# Storium — E-Commerce Monolith

## Proje Özeti
Storium, Django tabanlı, N-Katmanlı Mimari uygulayan bir monolitik e-ticaret platformudur.
Katmanlar: **Model → Repository → Service → DTO → View (Controller)**

## Hızlı Başlangıç
PostgreSQL ayakta olmalı (yerelde kurulum veya `docker compose up -d` — bkz. `docker-compose.yml`). `.env` içinde `DB_*` değişkenlerini doldurun.
...
- **DTO katmanı**: `@dataclass(frozen=True)`. ORM nesnesi içermez.
