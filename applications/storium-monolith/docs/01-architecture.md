...
### DTO Layer — `core/dtos/`
- `@dataclass(frozen=True)` tabanlı
- Input DTO: View → Service arası veri taşır (form verisi, query params)
...
urlpatterns = [
    ...
    path('', include('apps.catalog.urls')),           # / ve /kategori/ ve /urun/
    path('', include('apps.cart.urls')),              # /sepet/
    path('siparis/', include('apps.orders.urls')),    # /siparis/
]
```
