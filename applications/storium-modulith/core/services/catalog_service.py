...
class CatalogService:
    ...
    def get_category_nav_tree(self) -> list[CategoryNavNode]:
        hit = cache.get(_CATEGORY_NAV_CACHE_KEY)
        if hit is not None:
            return hit
        ...
        cache.set(_CATEGORY_NAV_CACHE_KEY, result, _CATEGORY_NAV_CACHE_TTL)
        return result
...
def invalidate_category_nav_cache() -> None:
    """Kategori ağacı seed/admin sonrası güncellenir."""
    cache.delete(_CATEGORY_NAV_CACHE_KEY)
